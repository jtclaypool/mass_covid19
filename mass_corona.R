...
test <- 'https://www.mass.gov/doc/covid-19-cases-in-massachusetts-as-of-march-17-2020/download'
...
dat_0313 <- build_dat_table(previous[2],14,19,'3/13/2020')
dat_0316 <- build_dat_table(previous[3],8,17,'3/16/2020')
dat_0317 <- build_dat_table(previous[4],8,17,'3/17/2020')
...
dat_current$population <- population$population[match(dat_current$counties,population$county)]
dat_current$percentage <- dat_current$covid.19/dat_current$population*100
save(dat_current,file = 'data/covid_cases.RData')


#current <- 
plot_usmap(data = dat_current[which(dat_current$date%in%as.Date("2020-03-17")),],values= 'covid.19',include=c("MA"),regions = 'counties',labels = T,aes(size=2))+
  ggplot2::scale_fill_continuous("Number of Covid-19 Cases")+
  ggplot2::theme(legend.position = "top",
                 plot.margin = unit(c(0,0,0,0),"cm"))+
  ggplot2::labs(title="Covid-19 Cases in Massachusetss March 17,2020")
#ggsave('plots/maps/mass_covid19_17032020.png',current,width = 70,height = 65,units = 'mm',scale = 10)
#source: https://www.massachusetts-demographics.com/counties_by_population

#percentage <- 
plot_usmap(data = dat_current[which(dat_current$date%in%as.Date("2020-03-17")),],values= 'percentage',include=c("MA"),regions = 'counties',labels = T)+
  ggplot2::theme(legend.position = "top",
                 plot.margin = unit(c(0,0,0,0),"cm"))+
  ggplot2::labs(title="Percentage of Population with Covid-19 Cases in Massachusetss March 17,2020")+
